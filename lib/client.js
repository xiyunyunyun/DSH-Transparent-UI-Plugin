		...
		function startSpotlight() {
			...
			*  React rerender must not restart their fade-in). */
			...
			const onMove = (event) => {
				...
				overTriggerNow = overTrigger(event.target) && spot.hasAttribute("data-dsh-inputbar");if (overTriggerNow) easeBack(spot);paint(session, event.clientX, event.clientY);
			};
			const onOver = (event) => {
				if (!hoverGated()) return;
				const spot = closestSpot(event.target);
				...
				current = spot;
				session = next;
				overTriggerNow = overTrigger(event.target) && spot.hasAttribute("data-dsh-inputbar");if (overTriggerNow) easeBack(spot);paint(next, event.clientX, event.clientY);
			};
			const onOut = (event) => {
				const spot = closestSpot(event.target);
