...
*  the spotlight disposer) share one document-wide query instead of three.
...
function spotElements() {
	if (spotCache === null || spotCache.some((el) => !el.isConnected)) spotCache = Array.from(document.querySelectorAll(SPOT_SELECTOR));
	return spotCache;
}
/**
...
function startBubbleAnchor() {
	...
	*  keeps the layout this frame paints already correct. */
	const pinPanelBubbles = () => {
		const bubbles = [];
		for (const bubble of document.querySelectorAll("[role=\"tooltip\"]")) if (bubble.closest(PANEL_SELECTOR) !== null) bubbles.push(bubble);
...
const seamCache = /* @__PURE__ */ new Map();
...
function stampSeam(seam, added) {
	...
	if (els === void 0 || added === null) {
		els = querySeam(seam);
		seamCache.set(seam, els);
	} else {
		const probe = seam.probe ?? seam.selector;
		if (els.some((el) => !el.isConnected) || added.some((root) => root.matches(probe) || root.querySelector(probe) !== null)) {
			els = querySeam(seam);
			seamCache.set(seam, els);
		}
	}
	let touched = false;
	for (const el of els) if (!el.hasAttribute(seam.attribute)) {
...
function stampAll(added = null) {
	...
	for (const seam of SEAMS) if (stampSeam(seam, added) && seam.attribute === "data-dsh-aqua-spot") spotsTouched = true;
...
const chatViewRoots = /* @__PURE__ */ new WeakSet();
...
function startSeamStamper() {
	...
	const observer = new MutationObserver((records) => {
		if (disposed) return;
		for (const record of records) for (const node of record.addedNodes) if (node instanceof Element && node.isConnected) (pendingAdded ??= []).push(node);
