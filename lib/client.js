	...
	const paint = (s, clientX, clientY) => {
		...
		raf = requestAnimationFrame(() => {
			...
			if (tiltGated() && tiltable(spot)) {
				...
				const originY = spot.hasAttribute("data-dsh-inputbar") ? local.top + local.height : local.top + local.height / 2;
				spot.style.transformOrigin = `${local.left + local.width / 2}px ${originY}px`;
