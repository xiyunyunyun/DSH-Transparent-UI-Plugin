...
var AquaPluginCard_module_default = {
	"check": "EG3s1W_check",
	"head": "EG3s1W_head",
	"title": "EG3s1W_title",
...
function AquaPluginCard(props) {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("li", {
		className: AquaPluginCard_module_default.card,
		children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
			className: AquaPluginCard_module_default.head,
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: AquaPluginCard_module_default.text,
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					className: AquaPluginCard_module_default.title,
					children: t("aqua.title")
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					className: AquaPluginCard_module_default.description,
					children: t("aqua.description")
				})]
			}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
				type: "button",
				className: AquaPluginCard_module_default.toggle,
				"aria-pressed": enabled,
				onClick: () => {
					setEnabled(!enabled);
				},
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					className: AquaPluginCard_module_default.check,
...
//#endregion
//#region \0dsh-css:C:\Users\Administrator\Desktop\dsh_workspace\DSH-Transparent-UI-Plugin\src\client\AquaAppearanceRow.module.aqcss
...
function FontPicker({ label, value, builtin, defaultName, cjk, t, onChange }) {
	...
	if (value !== "" && !builtin.includes(value) && !(system ?? []).includes(value)) entries.push({
		kind: "option",
		value,
		label: value
	});
	...
	*  clips against that ancestor, not the viewport — and enumerate once. */
	const toggle = () => {
		...
		if (open === false && trigger !== null && trigger !== void 0) {
			...
			} else setOpenUp(window.innerHeight - rect.bottom < 264 && rect.top > 264);
		}
		setOpen(!open);
	};
	(0, react.useEffect)(() => {
...
function startBubbleAnchor() {
	...
	*  the bubble elsewhere. */
	const triggerOf = (bubble) => {
		...
		while (prev !== null && prev.getAttribute("role") === "tooltip") prev = prev.previousElementSibling;
	...
	*  (or above) the pane, so no scroll region is ever inflated. */
	const pinBubble = (bubble) => {
		...
		const ar = triggerOf(bubble)?.getBoundingClientRect();
		if (ar === void 0 || ar.width === 0 && ar.height === 0) return;
	...
	*  keeps the layout this frame paints already correct. */
	const pinPanelBubbles = () => {
		for (const bubble of document.querySelectorAll("[role=\"tooltip\"]")) if (bubble.closest(PANEL_SELECTOR) !== null) pinBubble(bubble);
	};
	...
	*  and the phantom scrollable overflow never exists. */
	const mountObserver = new MutationObserver((records) => {
		for (const record of records) for (const node of record.addedNodes) {
			...
			for (const bubble of node.querySelectorAll("[role=\"tooltip\"]")) if (bubble.closest(PANEL_SELECTOR) !== null) pinBubble(bubble);
		}
	});
	...
	return () => {
		...
		for (const bubble of document.querySelectorAll("[role=\"tooltip\"]")) if (bubble.closest(PANEL_SELECTOR) !== null) {
...
*   paints no cards.
*/
function stampPluginViews() {
	for (const root of document.querySelectorAll("[data-slot=\"conversation.view\"] > *")) {
		if (root.querySelector("[data-slot^='conversation.chat'], [data-slot^='tool.call'], [data-composer-card], [data-dsh-inputbar]") !== null) {
			if (root.hasAttribute("data-dsh-view")) root.removeAttribute("data-dsh-view");
			if (root.hasAttribute("data-dsh-aqua-spot")) root.removeAttribute(SPOT_ATTR);
			continue;
		}
		...
		if (!spotted && !root.hasAttribute("data-dsh-aqua-spot")) root.setAttribute(SPOT_ATTR, "");
	}
}
/**
...
function mountWhale(host, dark) {
	...
	const sample = (img) => {
		...
		const lum = /* @__PURE__ */ new Float32Array(3600);
		for (let i = 0; i < 3600; i++) lum[i] = (.299 * data[4 * i] + .587 * data[4 * i + 1] + .114 * data[4 * i + 2]) / 255;
...
function inputbarPopover(spot) {
	return Array.from(spot.querySelectorAll("[role=\"dialog\"], [role=\"menu\"], [role=\"listbox\"]")).find((candidate) => {
...
function startSpotlight() {
	...
	*  a React rerender must not restart their fade-in). */
	...
	const clearSpot = (spot) => {
		...
		if (spot.hasAttribute("data-dsh-inputbar")) spot.removeAttribute("data-tilt-revealed");
		easeBack(spot);
	};
	...
	const paint = (s, clientX, clientY) => {
		...
		raf = requestAnimationFrame(() => {
			...
			if (glow !== null) {
				if (glowGated()) glow.style.backgroundImage = `radial-gradient(${GLOW_RADIUS}px at ${clientX - visual.left}px ${clientY - visual.top}px, var(--dsh-aqua-spot-color, ${GLOW_FALLBACK}), transparent 70%)`;
...
var AquaLayer = class {
	...
	*  glass panels stay frosted above the video. */
	...
	startBubbleFeed() {
		if (this.bubbleDisposer !== void 0) return;
		this.bubbleDisposer = startBubbleAnchor();
	}
};

//#endregion
//#region \0dsh-css:C:\Users\Administrator\Desktop\dsh_workspace\DSH-Transparent-UI-Plugin\src\client\aqua.module.aqcss
...
//#endregion
//#region \0dsh-css:C:\Users\Administrator\Desktop\dsh_workspace\DSH-Transparent-UI-Plugin\src\client\fonts.module.aqcss
...
function apply(ctx) {
	...
	const syncHostEnabled = () => {
		...
		if (typeof user === "object" && user !== null && !Array.isArray(user) && Object.prototype.hasOwnProperty.call(user, "enabled")) {
		...
		if (!legacyMigrationAttempted) {
			...
			if (legacyEnabled !== void 0) {
				...
				settings.set("enabled", legacyEnabled);
				return;
			}
		}
		layer.setEnabled(snapshot.value.enabled);
